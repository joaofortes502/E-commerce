...
// Mock do modelo Supplier usando SQLite em memória para testes
jest.mock('../models/Supplier', () => {
  ...
  return {
    ...
    create: jest.fn().mockImplementation((supplierData) => {
      return new Promise((resolve, reject) => {
        // Validações simuladas do modelo
    ...
    update: jest.fn().mockImplementation((id, updateData) => {
      return new Promise((resolve, reject) => {
        ...
        if (updateData.name && updateData.name.trim() === '') {
          reject(new Error('Nome não pode estar vazio'));
          return;
        }
    ...
    getProducts: jest.fn().mockImplementation((supplierId) => {
      return new Promise((resolve) => {
        if (parseInt(supplierId) === 1) {
          resolve([{ id: 1, name: 'Produto Teste', price: 100.00 }]);
        } else {
          resolve([]);
        }
      });
    ...
    getStats: jest.fn().mockImplementation((supplierId) => {
      return new Promise((resolve) => {
        if (parseInt(supplierId) === 1) {
          resolve({ total_products: 1, total_stock: 10, stock_value: 1000 });
        } else {
          resolve({ total_products: 0, total_stock: 0, stock_value: 0 });
        }
      });
    })
  };
